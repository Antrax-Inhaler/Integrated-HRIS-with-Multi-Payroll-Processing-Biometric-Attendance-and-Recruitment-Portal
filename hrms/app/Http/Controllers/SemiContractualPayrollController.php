...
class SemiContractualPayrollController extends Controller
{
    function showPayroll() 
    {
        $payrolls = Payroll::with(['payrollItems.member'])
            ->where('type', 'semi-monthly') // Filter payrolls by type 'monthly'
            ->whereHas('payrollItems.member', function ($query) {
                $query->where('employment_status', 'contractual-semi'); // Filter members by 'contractual-semi'
            })
            ->get();
    
        return view('admin.payroll_semi_contractual_monthly', compact('payrolls'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $payPeriod = $request->pay_period; // Get the selected pay period
    
        // Set the date range based on the selected pay period
        if ($payPeriod === 'first') {
            $dateFrom = $selectedMonth->copy()->startOfMonth()->startOfDay(); // 1st of the month
            $dateTo = $selectedMonth->copy()->day(15)->endOfDay(); // 15th of the month
        } else {
            $dateFrom = $selectedMonth->copy()->day(16)->startOfDay(); // 16th of the month
            $dateTo = $selectedMonth->copy()->endOfMonth()->endOfDay(); // End of the month
        }
    
        // Create a new payroll record for the selected month and pay period
        $payroll = Payroll::create([
            'ref_no' => 'PAY-' . $selectedMonth->format('Y-m-d-H-i-s'), // Creates ref_no as PAY-YYYY-MM-DD-HH-MM-SS
            'month' => $selectedMonth->format('Y-m'), // Assuming you have a 'month' field
            'date_from' => $dateFrom, // Include date_from
            'date_to' => $dateTo,     // Include date_to
            'type' => 'semi-monthly',
            'status' => 'computed',
        ]);
    
        // Fetch members who are verified and have employment status 'contractual-semi'
        $members = Member::where('is_verified', 1)
                         ->where('employment_status', 'contractual-semi')
                         ->get();
    
        foreach ($members as $member) {
            // Pass $payroll as the first argument in the method call
            $this->updatePayrollItem($payroll, $dateFrom, $dateTo, $member, $selectedMonth);
        }
    
        return redirect()->route('admin.payroll_semi_contractual_monthly')
                         ->with('message', 'Payroll computed successfully.');
    }
    ...
    private function updatePayrollItem($payroll, $dateFrom, $dateTo, $member, Carbon $selectedMonth)
    {
        ...
        foreach ($attendanceRecords as $date => $logs) {
            ...
            if ($amIn && $amOut) {
                ...
                if ($amIn->datetime_log->gt(Carbon::parse($date)->setTime(8, 0, 59))) {
                    $lateCount++;
                }
            }
        ...
        // Calculate actual working days for the semi-monthly period (excluding Sundays and holidays)
    // Get the first and last day of the month from the selected month
$firstDayOfMonth = $selectedMonth->copy()->startOfMonth();
$lastDayOfMonth = $selectedMonth->copy()->endOfMonth();

// Calculate working days based on the full month, excluding Sundays and holidays
$workingDays = $firstDayOfMonth->diffInDaysFiltered(function (Carbon $date) {
    return $date->dayOfWeek !== Carbon::SUNDAY && !$this->isHoliday($date->toDateString());
}, $lastDayOfMonth->copy()->addDay());

    
        // Adjust hourly rate based on actual working days
        $hourlyRate = $member->salary / ($workingDays * 8); // Assuming 8 hours per day
    
        // Calculate gross salary for the semi-monthly period
        $grossSalary = ($totalHoursWorked * $hourlyRate) + $totalHolidayPay;
        $bonuses = $this->calculateBonuses($member);
    
        // Add AddCom and PeraAca if applicable for this period
        $monthYear = Carbon::parse($dateFrom)->format('Y-m-01');
        $addComTotal = AddCom::where('member_id', $member->id)
            ->where('month_year', $monthYear)
            ->sum('amount');
        $peraAcaTotal = PeraAca::where('member_id', $member->id)
            ->where('month_year', $monthYear)
            ->sum('amount');
        ...
        // Get deductions, including late deduction
        ...
        $lateDeduction = $deductionsData['late_deduction'];
    
        // Calculate net salary
        $netSalary = $grossSalary + $bonuses - $totalDeductions;
    
        // Store payroll item with the calculated data
        PayrollItem::updateOrCreate(
            ['payroll_id' => $payroll->id, 'member_id' => $member->id],
            [
                'basic_salary' => $member->salary,
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('payrollCS.show')->with('success', 'Payroll deleted successfully.');
    }
...
public function show($id)
{
    ...
    return view('admin.payroll_semi_contractual_monthly_detail', compact('payroll'));
}
...
public function recalculate(Request $request, $payrollId)
{
    ...
    // Fetch members who are verified and have employment status set to 'contractual-semi'
    $members = Member::where('is_verified', 1)
                     ->where('employment_status', 'contractual-semi')
                     ->get();
    ...
    return redirect()->route('admin.payrollCS.show', $payroll->id)
