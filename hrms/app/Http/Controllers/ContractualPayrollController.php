...
class ContractualPayrollController extends Controller
{
    // Function to compute payroll for contractual employees
    function showPayroll() 
    {
        $payrolls = Payroll::with(['payrollItems.member'])
            ...
            ->whereHas('payrollItems.member', function ($query) {
                $query->where('employment_status', 'contractual'); // Filter members by 'contractual'
            })
            ->get();
    
        return view('admin.payroll_contractual_monthly', compact('payrolls'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $dateFrom = $selectedMonth->copy()->startOfMonth()->startOfDay(); // Start of the selected month
        $dateTo = $selectedMonth->copy()->endOfMonth()->endOfDay(); // End of the selected month
        // Create a new payroll record for the selected month with a generated ref_no
        ...
        // Fetch members who are verified and have employment status set to 'contractual'
        ...
        return redirect()->route('admin.payroll_contractual_monthly')->with('message', 'Payroll computed successfully.');
    }
    ...
    private function updatePayrollItem($payroll, $dateFrom, $dateTo, $member)
    {
        ...
        // Calculate dynamic working days (excluding Sundays)
        $workingDays = Carbon::parse($dateFrom)->diffInDaysFiltered(function (Carbon $date) {
            return $date->dayOfWeek !== Carbon::SUNDAY;
        }, Carbon::parse($dateTo)->copy()->addDay());
    
        // Salary and Deductions Calculations
        $hourlyRate = $member->salary / ($workingDays * 8); // Using dynamic working days
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('payrollCM.show')->with('success', 'Payroll deleted successfully.');
    }
...
public function show($id)
{
    ...
    return view('admin.payroll_contractual_monthly_detail', compact('payroll'));
}
...
public function recalculate(Request $request, $payrollId)
{
    ...
    return redirect()->route('admin.payrollCM.show', $payroll->id)
...
public function updateStatus(Request $request, $id)
{
    ...
    return redirect()->route('admin.payroll_contractual_monthly')
