...
<section class="home-section" style="width: calc(100% - 58px); overflow:scroll">
   ...
   <h1>Contractual Monthly</h1>

  <!-- Button with Lordicon -->
<div class="payroll-button-container">
    <button class="payroll-round-button" id="togglePayrollCMFormButton">
    ...
    <span class="payroll-hover-label">Create Commission Payroll</span>
</div>

<!-- Payroll Commission Form (Initially Hidden) -->
<div class="payroll-card" id="payrollCMCard">
    <h1 class="payroll-header">Contractual Payroll</h1>
    <form action="{{ route('admin.payrollCM.store') }}" method="POST">
        @csrf
...
<script>
    // Toggle visibility of the commission payroll form
    document.getElementById('togglePayrollCMFormButton').addEventListener('click', function () {
        const cmCard = document.getElementById('payrollCMCard');
        cmCard.style.display = (cmCard.style.display === 'none' || cmCard.style.display === '') 
                                ? 'block' : 'none';
    });
</script>
   ...
   <table class="table table-bordered">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="{{ route('admin.payrollCM.show', $payroll->id) }}" class="btn btn-info">View Details</a>
                
                <!-- Update Status Form -->
                <form action="{{ route('admin.payrollCM.updateStatus', $payroll->id) }}" method="POST" style="display:inline-block;">
                    @csrf
                ...
                <!-- Delete Form -->
                <form action="{{ route('admin.payrollCM.destroy', $payroll->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this payroll?');">
                    @csrf
                    @method('DELETE')
