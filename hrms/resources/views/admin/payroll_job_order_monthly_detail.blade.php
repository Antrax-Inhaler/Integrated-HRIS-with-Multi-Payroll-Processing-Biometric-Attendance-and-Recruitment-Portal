...
<section class="home-section" style="width: calc(100% - 58px); overflow-x:scroll">
    <h1>Payroll Details from {{ \Carbon\Carbon::parse($payroll->date_from)->format('F j, Y') }} to {{ \Carbon\Carbon::parse($payroll->date_to)->format('F j, Y') }}</h1>
    <br>
    <style>
        .button-navigations{
            display: flex;
    ...
    <div class="button-navigations">
        <form action="{{ route('admin.payrollJO.recalculate', $payroll->id) }}" method="POST">
            @csrf
            <button type="submit" class="btn btn-warning">Recalculate Payroll</button>
        </form>
        <div class="left-buttons">
        <a href="{{ route('admin.payrollJO.print', $payroll->id) }}" target="_blank" class="btn btn-success">
            Print Payroll
        </a>
        <a href="{{ route('admin.payslipJO.printAll', $payroll->id) }}" target="_blank" class="btn btn-primary mb-3">
            Print All Payslips
        </a>
    </div>
</div>
