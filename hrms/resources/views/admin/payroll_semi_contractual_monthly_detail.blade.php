...
<div class="button-navigations">
    <form action="{{ route('admin.payrollCS.recalculate', $payroll->id) }}" method="POST">
        @csrf
        <button type="submit" class="btn btn-warning">Recalculate Payroll</button>
    </form>
    <div class="left-buttons">
    <a href="{{ route('admin.payrollCS.print', $payroll->id) }}" target="_blank" class="btn btn-success">
        Print Payroll
    </a>
    <a href="{{ route('admin.payslipCS.printAll', $payroll->id) }}" target="_blank" class="btn btn-primary mb-3">
        Print All Payslips
    </a>
</div>
</div>
